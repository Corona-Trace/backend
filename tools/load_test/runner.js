...
function createUser() {
    ...
    return axios.post(API_ROOT+'/users', userData)
    .then(d => d.data)
    .then(reply => {
        if (reply === "OK") {
            return userData
...
function sendPoint(point, user) {
    ...
    return axios.post(API_ROOT+'/usersLocationHistory', trace)
            .then(k => k.data)
}
...
function runScenario(scenario) {
    ...
    return createUser()
    .then(user => {
        return Promise.all(trail.map(point => {
            return sendPointLimited(point, user)
            .then(k => {
                console.log(k, point)
                return k
            })
...
function loadAndRun(s) {
    ...
    return prepareScenarios(s.interval, s.path)
    .then(scenarios => {
        const limiter = new Bottleneck({
...
const scenario = {
    users: 100,
    interval: 100, //ms,
    path: "2020-04-11/trackjobs2.json/part-00025-2476ec3d-2037-462e-bbe1-00fd21cd0192-c000.json"
}
